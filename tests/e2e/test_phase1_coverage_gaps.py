...
def test_dinf_accumulation_total_mass_bounded_by_cell_count():
    ...
    rows, cols = 5, 7
    z = np.tile(np.arange(cols, 0, -1, dtype=np.float32), (rows, 1))
    dem = _make_dem(z)
    fd = dem.flow_direction(method="dinf")
    acc = fd.accumulate()
    arr = acc.read_array()
...
def test_rho8_different_seeds_can_diverge():
    ...
    dem = _make_dem(z)
    seen = {
        tuple(dem.flow_direction(method="rho8", seed=s).read_array().ravel())
...
class TestLocalMinima8EdgeCases:
    ...
    def test_interior_nan_cell_excluded(self):
        ...
        z = np.array([[5, 5, 5], [5, np.nan, 5], [5, 5, 5]], dtype=np.float64)
    ...
    def test_separate_nodata_mask_is_honoured(self):
        ...
        z = np.array([[5, 5, 5], [5, 1, 5], [5, 5, 5]], dtype=np.float64)
...
@pytest.mark.parametrize("method", ["dinf", "mfd_quinn", "mfd_holmgren"])
